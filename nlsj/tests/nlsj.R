...
# Does not work without start. Does start get used?
...
## directly -- specified start. Gets different answer
fm2 <- nlsj(density ~ Asym/(1 + exp((xmid - log(conc))/scal)),
           data = DNase1, weights = wts,
           start = list(Asym = 3, xmid = 0, scal = 1))
summary(fm2)
if (is.character(all.equal(coef(summary(fm2)), coef(summary(fm1a)), tolerance = 1e-6))) {
      warning("Coefficients of fm2 and fm1a NOT equal")
}
if (is.character(all.equal(residuals(fm2), residuals(fm1a), tolerance = 1e-5))) {
  warning("Residuals of fm2 and fm1a NOT equal")
}

if (is.character(all.equal(fitted(fm2), fitted(fm1a), tolerance = 1e-6))) {
  warning("Fitted of fm2 and fm1a NOT equal")
}
...
if (is.character(all.equal(ft, fitted(fm1), tolerance = 1e-6))) {
  warning("Coefficients of fm3 and fm1 NOT equal")
}

# sign of residuals is reversed
r <- with(DNase1, -residuals(fm3)/sqrt(wts))
aer<-all.equal(r, residuals(fm1), tolerance = 1e-5)
print(aer)
fm3a <- nlsj(~ sqrt(wts) * (density - Asym/(1 + exp((xmid - log(conc))))),
            data = DNase1, start = list(Asym = 3, xmid = 0))
cat("Suppressed call to anova(fm3a, fm3) due to unresolved error\n")
#anova(fm3a, fm3)
...
summary(fm4)
# cf <- coef(summary(fm4))[c(3,1,2), ] # ?? suppressed -- error
# rownames(cf)[2] <- "Asym"
...
# ## 220527A
# fm1nls<- nls(density ~ SSlogis(log(conc), Asym, xmid, scal),
#             data = DNase1, weights = DNase1$wts)
# summary(fm1nls)
# pfm1nls <- profile(fm1nls)
# pfm1nls
# 
# ## check profiling
# pfm1 <- profile(fm1)
