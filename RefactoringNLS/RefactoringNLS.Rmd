...
(NOTE: For the purposes of this article, we use a developmental version of package
`nlsr` labelled `nlsr2`.)
...
The function `resid()` (an alias for `residuals()`) `nlsModel()` is WEIGHTED, as are those in the
...
modelling tools, and within `nls()` itself. This is highlighted by the case of
...
Indexed parameters -- those whose names have subscripts on some index variable -- 
...
Nonlinear models, apart from those using the "plinear" algorithm, need all parameters specified.
