...
An important consideration is the  maintainability of
...
for estimating nonlinear statistical models. The function dates to the 1980s and the work related
to @bateswatts in S (see https://en.wikipedia.org/wiki/S_%28programming_language%29). 
...
This amounts to admitting R cannot solve well-posed problems unless data is polluted
with errors. 
...
stabilizations, but integration with all the features of `nls()` has proved difficult.
...
`nlsr::nlxb()` displays the singular values of the Jacobian, so the `nls()` error message is correct,
but unhelpful in obtaining a solution.
...
values. Note that these are **displayed** in a single column in
...
of `nls()` (and, by extension, `minpack.lm::nlsLM`) mostly use a numerical approximation. 
...
would avoid a singular Jacobiam because $b$ and $d$ were equal in value. 
...
but some information that would be useful is not presented obviously. Moreover, the
...
Yet the explicit model `y ~ a*x + b` does not work with the linear modeling 
function `lm()`, which requires this model to be specified as `y ~ x`.

Within `nls()`, consider the 
following FOUR different calling sequences for the same problem, though an intuitive choice,
labelled fm2a, will not work. In this failed
