...
the work related to @bateswatts in S (see *https://en.wikipedia.org/wiki/S_%28programming_language%29*). 

The `nls()` function has a remarkable and comprehensive set of capabilities for
estimating nonlinear models that are expressed as **formulas**. In particular, we note that it
...
A previous issue with `nls()` that prevented it from providing parameter 
...
"convergence" of the underlying **algorithm** is the Relative Offset 
Convergence Criterion (see @BatesWatts81). This projects the proposed step 
...
In 2021, one of us (J. Nash) proposed that `nls.control()` have an additional
...
The convergence criterion above, the principal termination control of `nls()`, 
leaves out some possibilities that could be useful for some problems. The 
package `nlsr` (@nlsr2019manual) already offers both 
...
work-around is the Levenberg-Marquardt stabilization (see @Marquardt1963,
@Levenberg1944, @jn77ima), and versions of it have been implemented in 
packages `minpack.lm` and `nlsr`.
...
`nlsr::nlxb()` displays the singular values of the Jacobian, which confirm that 
the `nls()` error message is correct, but this is not helpful in obtaining a
solution.
...
The Jacobian used by `nlsr::nlxb()` is, by default, computed from analytic
expressions, while that of `nls()` mostly uses a numerical approximation. Some
`selfStart` models do provide "gradient" information. `minpack.lm::nlsLM()` 
invokes `numericDeriv()` in its local version of `nlsModel()`, but it appears 
to use an internal approximate Jacobian code from the original Fortran 
`minpack` code, namely, `lmdif.f`. Such differences in approach can lead to 
...
Note that the `selfStart` models in the base R `./src/library/stats/R/zzModels.R` 
file and in package `nlraa` (@MiguezNLRAA2021) may
provide the Jacobian in the "gradient" attribute of the "one-sided" formula that 
defines each model, and these Jacobians may be the analytic forms. The `nls()` 
function, after computing the "right hand side" or `rhs` of the residual, then 
checks to see if the "gradient" attribute is defined, and, if not, uses
`numericDeriv` to compute a Jacobian
...
returned by the `residuals()` function are weighted, and the `fitted()` or
`predict()` function are used to compute raw residuals.


## Feature: Subsetting

`nls()` accepts an argument `subset`. This acts through the mediation of
`model.frame` and is not clearly obvious in the source code files
`/src/library/stats/R/nls.R` and `/src/library/stats/src/nls.C`. 
...
`na.action` is an argument to the `nls()` function, but it does not appear obviously 
in the source code, often being handled behind the scenes after referencing the
option `na.action`. A useful, but possibly dated, description is given in:
...
`na.pass` simply passes on data "as is", while `na.fail` will essentially stop 
...
object, and the element `model` is NULL unless the calling argument `model` is 
TRUE. (Using the same name could be confusing.) However, the model frame is 
used within the function code in the form of the object `mf`. We feel that 
...
suppress saving this workspace (`.RData`) on exit; otherwise, any such file in the 
...
parameters. `nls()` offers a special class of modeling functions called
**`selfStart`** models. There are a number of these in base R
...
dangerous. Moreover, we believe that it would be helpful to have `selfStart` 
models that allow users to explicitly provide values other than those suggested 
for the starting parameters.

Other concerns are revealed by the example below. Here, the `SSlogis` 
`selfStart` function is used to generate a set
...
another form of the model, namely, $y \,\sim\, b1\,/\,(1 \,+\,
b2\,*\,exp(-b3\,*\,t))$.
...
`ls()` command, making them difficult to access.

In the event that a `selfStart` model is not available, `nls()` sets all the 
...
The `nls` object contains some elements that are awkward to produce by other 
...
As we made our comparisons, we noted several potential updates to the `nlsr`
documentation as well as that for `nls()`.

### Weights in returned functions from nls()

As already noted, the function `resid()` (an alias for `residuals()`) is 
WEIGHTED, as are those in the structured object `m` returned by `nls()` or
`minpack.lm::nlsLM`, e.g., `ans$m$resid()`. The function `nlsModel()`, which 
we have had to extract from the base R code and explicitly `source()` because 
...
embedded in the Fortran of the `nlminb` routine that is called to execute the 
...
The variable projection method (@Golub1973, @OlearyRust13) is generally much more 
...
modeling tools, and within `nls()` itself. This is highlighted by the case of
...
problem, plus an intuitive choice, labelled `fm2a`, that does NOT work. In this
failed attempt, putting the `Asym` parameter in the model causes the `plinear`
...
`nls()` can impose bounds on parameters, but only if the `port` algorithm is used.
...
for package `nlsr`. Package `minpack.lm` also includes provision for bounds, but 
the following script
<!-- , which we do not run here for reasons of space,  -->
...
The `nls()` code is structured in a way that inhibits both maintenance and 
...
- to provide a packaged version of `nls()` (call it `nlsalt`) coded entirely in R 
  ...
  in @PkgFromRbase22. The R version can either serve as the operational code or
...
add `algorithm="plinear"` to the standard call, making errors more likely. 
...
for the output of different `algorithm` options to be consistent. However, we have 
...
to every fitted point, but `a[Strip]` and `b[Strip]` apply only to data indexed, which
could be an experimental run number, or some other categorization. 

Given that `[ ]` provides the R method to index arrays, the presence of square
brackets in a model
formula signals indexing. The question is how to perform the correct calculations.

Our view is that inclusion of the indexing **within** `nls()` or similar tools 
...
indexed parameters renamed e.g., `a[2]` becomes `a2` as at present with `nls()`.
...
*https://github.com/ArkaB-DS/nlsCompare* .
...
functions) that are created by tools such as `nls()` should have minimal
...
- Weights, subsets, and various contextual controls (such as that for the 
  `na.action` option) are similarly taken from the first available source, 
...
- All iterative codes should return the best solution they have found so far, 
  ...
  This modification could be made to `nls()` in the short term, correcting the 
...
`nls()` and `nlsr::nlxb()` is that `nls()` 
builds a large infrastructure, especially the `nlsModel()` function,  from 
...
of us (JN) established almost 50 years ago with the software that became @cnm79,
separating setup, solver, and post-solution analysis phases of computations. 
There is, however, the wrapper `wrapnlsr()` that solves the nonlinear estimation problem with the solver of `nlsr` then calls `nls()` to build the object of 
class `nls` for which profile log-likelihood information can be computed.
...
clarify aspects of `selfStart` models and improve package `nlsr` in that respect.
Colin Gillespie (package `benchmarkme`) has been helpful in guiding our attempts
