...
Various packages, in particular our `nlsr`, provide alternative capabilities.
...
from the Comprehensive R Archive Network (*https://cran.r-project.org*), hereafter
abbreviated CRAN.
...
in S (see *https://en.wikipedia.org/wiki/S_%28programming_language%29*). 
...
of the deficiencies discovered over the long period that the `nls()`
...
improvements and preserved legacy results. On the other hand, working
with `nls()` suggested ways our own `nlsr` package could be improved
...
with a limited consideration of some other tools that exist for 
nonlinear least squares calculations. A hope is that we will encourage
others to engage, possibly with us, to find ways to bring improvements.
We note that the referees' comments have been most helpful to focus
the discussion here in ways that illustrate the best aspects of open
source software development. We believe it is desirable to consolidate
tools where reasonable so that users have an easier task in choosing
which to use, but such consolidation demands careful and wide-ranging
discussion of options. It is much simpler to create new packages than
to bring together and simplify existing ones.

### Tools considered

Besides the base-R `nls()` function, we consider some packages that 
are available on the CRAN repository.
...
we use 1-line result display functions from package `nlsr`, namely `pnls0()` 
and `pshort()`.
...
However, `nls()` also offers the option of using the "port" library algorithm
...
pnls0(ugslnls1) # to get sum of squares
sgslnls1<-try(gsl_nls(fn = frms, data = weeddf,  start = sts1))
pnls0(sgslnls1) # Use summary() to get display
...
As pointed out by one of the referees, such behaviour is consistent
with other modeling functions such as `lm()`.
...
what is called the `gradient` element in the returned solution for these solvers.
...
parameters reported at this saddle point, `nlsr::nlxb()` cannot proceed. We also 
...
We consider the code inside package `nlsr` to be competently coded, with
the advantage over `nls()` of the Levenberg-Marquardt
stabilization in common with `minpack.lm` and `glsnls`. However, our principal 
message is that we obtain diagnostic information in the form of the singular values
of the Jacobian that allow us to try alternative starts, algorithms or programs in 
cases where there are indications of early termination.
...
famous Rosenbrock banana-shaped valley (@Rosenbrock1960, 
also https://en.wikipedia.org/wiki/Rosenbrock_function) has 
$$ r_1(p) = (1 - p_1)$$
$$ r_2(p) = 10 (p_2 - {p_1}^2)$$
...
is missing. Note function `nlsr::pnls0()` for a 1-line display of
...
pnls0(hobgsl) # using analytic Jacobian
```
...
```
However, a better approach is to make the call to `nls()` with `model=TRUE`, 
...
model or provide explicit values, but the `selfStart` model is used in the formula 
for the `nlxb()` call.
...
names preceded by ".".  A referee has indicated this behaviour is to avoid name 
...
specify where the solution will be.
...
One of the referees pointed out that nlsr::nlfb() is quite slow on a
"large" case of the Penalty 1 test problem of @More1981TUO. We agree. 
However, we would suggest that problems in large numbers of parameters
will be problematic for Gauss-Newton methods because they create large
computational matrices for the Jacobian or its inner product. The method
"Rcgmin" from package `optimx` (@p-optimx) can be orders of magnitude
faster, but is entirely written in R. Note that "Rcgmin" is a much more
reliable conjugate gradient minimizer than method "CG" of `optim()`. One
of us created both, but now deprecates the latter. It should be noted
that timings of CG methods are more sensitive to small differences in
setup and arithmetic than Gauss-Newton methods.
