...
Technically it is the Jacobian that is singular. A
referee noted that R in this case cannot distinguish scalar and vector functions,
which would yield partial derivatives that are a gradient or Jacobian respectively.
...
While default `gsl_nls()` gives much the same results as `nlsLM()`, uncommenting the
...
but `nlsr` has its premature terminations too. Our principal message is that we obtain 
...
is missing. Note function `nlsr::pnls()` for a 1-line display of
the results.
...
which is intended for problems specified as formulas.
...
```
However, since the evaluating environment might not exist (thanks to a referee
for noting this possibility) a better approach is to make the call to `nls()` 
...
for customized tools. Coding in R is intended to support such uses.
...
However, we note that problems in large numbers of parameters
will be problematic for Gauss-Newton methods because they create large
computational matrices for the Jacobian or its inner product. This was
confirmed by comparing `nlsr::nlfb()` and `minpack.lm::nls.lm()` on
the Penalty 1 problem of various sizes, where the latter, which is 
compiled Fortran internally, was moderately faster. However, the method
"Rcgmin" from package `optimx` (@p-optimx) can be much 
faster, despite being entirely written in R, and is a much more
