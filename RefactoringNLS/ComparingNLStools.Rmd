...
**minpack.lm** (@minpacklm12), gslnls (@gslnls23), and nls2 (@nls2-22).

There are a number of other CRAN packages worth commentary, and we will give
some capsule comments later. It is likely that there are a number of other 
tools that could be relevant to
...
can modify to deal with their own problems is essential. The presence of vignettes, 
...
nonlinear least squares problems from the U.S. National Institute for Standards
and Technology for testing solvers. We caution that the problems are generally 
poorly conditioned and may not represent real-world situations.
...
`nlsic` (@nlsic-manual) -- This solves non linear least squares problems with optional
...
`nlsMicrobio` (@nlsMicrobio-manual) -- Data sets and nonlinear regression models dedicated 
to predictive microbiology, including a vignette, by authors of the
...
of nonlinear models, particularly using `nls()`. The vignette is actually a journal article, 
and the authors have considerable experience in the subject.
...
here is to minimize an objective which is modified from the traditional sum of squared residuals.
...
`nls2` (@nls2-22) -- Nonlinear least squares by brute force has some similar
motivations, but uses `nls()` within multiple trials. The package and its
author have a long history in the R community.

`nlstac` (@m-nlstac) -- A set of functions implementing the algorithm described in Fernandez 
    Torvisco et al. (2018) for fitting separable nonlinear regression curves. The special 
    class of problem for which this package is intended is an important and difficult one.
    No vignette is provided, unfortunately.

`easynls`  -- Fit and plot some nonlinear models. Thirteen models are treated, but there is minimal 
documentation and no vignette. Package `nlraa` is to be preferred.

`nlraa`  (@MiguezNLRAA2021) -- a set of nonlinear *selfStart* models. Most include analytic Jacobian
code.

`optimx` (@p-optimx) -- not really a nonlinear least squares method, but has general optimizers
that can be applied to minimize a nonlinear function e.g., a nonlinear sum of 
squares, as well as other objectives. This is not generally recommended if nonlinear least 
squares programs can be easily used, but provides a way to approach problems when others are
unavailable.
...
since the sumsquares loss function has an identical value. In both cases, the
...
function) defined by the sum of squares of the residuals by 
...
a linear combination of these ideas. In low precision, some elements of 
...
situation in computing $\delta$. Below we see `nls()` does poorly on this 
example problem. Note that `nlsr`
offers function `pnlm0()` to give a 1-line display of results of class `nls`,
and `pshort()` for `nlsr`.
...
that the scaled logistic has the largest ("least singular") ratio. 
...
require(minpack.lm); require(nlsr); require(gslnls)
...
with suggested addition of small perturbations to the data. This amounted to 
...
approached with nonlinear least squares. Moreover, the matter 
...
vector onto the gradient and estimates how much the sum of squares loss function 
should decrease. This estimate is divided 
by the current size of the loss function to avoid scale issues.
When we have "converged", the estimated relative
...
additional parameter `scaleOffset` with a default value of zero. Setting it to a small 
...
since version 4.1.0; it has been a part of `nlsr` since it was introduced.

More general termination tests can be used. `nlsr` has a **small sum of 
...
examination of the code. We prefer to use the number of times the residuals or 
the jacobian have been computed and put upper limits on these. Our codes exit (terminate) 
when these limits are reached. Generally we prefer larger limits than the 
default `maxiter = 50` of `nls()`, avoiding the risk of stopping early. 
...
this matrix lets us embed this attribute in the **residual** function as well,
...
and its wrapper R code function `numericDeriv()`, part of `./src/library/stats/R/nls.R`
...
can lead to different behaviour, though in our experience these differences are usually
very minor.

However, minor numerical differences can give annoying results with ill-conditioned
problems. 
...
is intended to solve our example problem **Logistic3T**.  This example is one 
where the `getInitial()` function (used to find
starting values) actually calls `nls()` recursively with a non-default algorithm
to get "starting" parameters. Then it returns to running the default algorithm
unnecessarily. Such circular references are, in our view, prone to creating errors
in code maintenance. 
To provide simpler starting parameters, the function `SSlogisJN`
is now part of package `nlsr`.

Users may also want to use `selfStart` models with explicit starting values 
...
and others save many quantities in "hidden"
...
`nls()` and other class `nls` tools accept an argument `subset`. This acts through the 
mediation of `model.frame`, which not obvious in the 
...
`na.exclude` does much of the same for solver computations, but keeps the rows with NA 
...
name could
be confusing.) Despite this, the model frame is used within the function code in the form 
of the object
...
?? Arkajyoti: I think this example may get too long or be too unclear.
...
the asymptote parameters are an illustration.However, setting up the 
...
with all its current capabilities. Unfortunately, `nls()` is part of the base R distribution, 
...
The aspects of `nls()` that have given us the most trouble follow:

- R tools often presume data and parameters needed are available in an accessible environment. 
  As long as the correct environment is used, this provides a compact syntax to invoke the
  calculations. The danger is that the wrong data is accessed if the internal search finds a
  valid name that is not the object we want, or if `subset` or `na.action` settings modify
  the selection, or `weights` are applied.
...
- The class `nls` structure simplifies calls with its rich set of functions, but
  also adds to the task of following what has been done. A design that isolates the 
  setup, solution, and post-solution parts of complicated calculations reduces the 
  number of objects that must be kept in alignment. 
...
be moving to incorporate them. These capabilities are available now by using several tools,
but it would be helpful if they were unified.
