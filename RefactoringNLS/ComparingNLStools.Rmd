...
The rich features of this function are weakened by several deficiencies and
...
This article considers the differences in goals, approaches, and features of
...
`nls()` is the tool in base R, the primary software distribution
...
There are some other CRAN packages worth commentary, and we will give
some capsule comments later. There are likely several other 
...
## Other CRAN packages considered
...
motivation, but uses `nls()` within multiple trials. The package and its
...
`optimx` (@p-optimx) -- not really a nonlinear least squares method, but has general
optimizers that can be applied to minimize a nonlinear function e.g., a nonlinear sum of 
squares, as well as other objectives. This is not generally recommended if nonlinear 
least squares programs can be easily used, but provides a way to approach problems 
when others are unavailable.

## An illustrative example

The Hobbs weed infestation problem (@cnm79 [page 120]) is a growth curve modeling 
task that is seemingly straightforward but turns out to be quite nasty. Because
...
The data and a graph of it are given below.
...
to compute it precisely is considerable. Many workarounds have been 
...
was the first to actually incorporate the ideas into a practical computer
program. 
...
feature at several points below. However, if a `selfStart` model 
...
faraway::sumary(unlm1) # Use summary() to get display
...
- `nlsLM()` includes bounds in the standard call, but we have observed cases
   ...
   it should be noted that all programs have some weakness regarding constrained
   optimization. Software creators have to work with
