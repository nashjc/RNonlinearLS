...
the most commonly used. We will use "Marquardt", as we believe he
...
choices was suggested as a compromise. Various choices for
...
```{r ex02minpack}
library(minpack.lm)
# ??not needed -- library(nlsr) # for pnlm0
unlm1<-try(nlsLM(formula=frmu, start=stu1, data=weeddf))
# summary not exported by faraway??
# faraway::summary(unlm1) # Use summary() to get display
...
?? Can we simplify or suggest use of `str()` ??
...
- R tools often presume data and parameters needed are available in an 
  accessible environment. As long as the correct environment is used, this provides 
  a compact syntax to invoke the
  calculations. The danger is that the wrong data is accessed if the 
  internal search finds a
