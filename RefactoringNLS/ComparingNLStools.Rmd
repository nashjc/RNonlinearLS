...
this comparison in the Bioconductor (??ref) collection and on repositories like
...
R tools are our interest, but the tools we consider often use other programming
...
programming languages. Changes to one or the other language or its library support
...
(?? ref). This is often very effective and efficient, but for a quite large 
...
direction $\delta$ rather than a unit step, give rise to a panoply of variant
...
The following show `nls()` does poorly on this example problem. 
...
and `gslnls::gsl_nls()`. We use default methods and controls in these examples. 
...
summary(sgslnls1) # Use summary() to get display
tgslnls1<-try(gsl_nls(fn = frmt, data = weeddf,  start = stt1))
summary(tgslnls1) # Use summary() to get display
```
...
When using formula specification of the model, `nls()`, `minpack.lm::nlsLM()`
...
When The Jacobian code as 
the "gradient" attribute of the output of the Jacobian function. This 
lets us embed the code 
...
Generally `nls()`, `nlsLM()` and `gsl_nls()` use a fairly 
simple forward difference approximation of derivatives for the Jacobian
from the base-R `stats` package called `numericDeriv()`. A central 
approximation can be specified in control parameters. 
Package `nlsr` provides, via appropriate calling syntax, four approximation 
...
of `selfStart`, but the construction of them is non-trivial. Their use with
`nlsr` is described in the "Introduction to nlsr" vignette. However, since `nlsr`
generally can use very crude starting values, we have rarely needed them. 
...
routine that is called to execute the "port" algorithm, but the factor of 2 discrepancy is nonetheless
...
effective than general approaches in finding good solutions to nonlinear least squares 
...
- to explain what we do, either in comments, examples, or separate maintainer documentation.
...
Our view is that inclusion of the indexing **within** `nls()` or similar tools introduces 
...
a wrapper could process output to reconstitute the indexing of parameters. 
...
two decades. Given its importance to R, the focus of discussion should be the measures
needed to secure its continued operation for legacy purposes and how that may be approached.
