# File: badJ2.R
# A problem illustrating poor numeric Jacobian ?? 20211217 does not fail??
...
c<-0.68
cat("c =",c,"\n")
yexact<-10*a*(8+b*log(1-0.049*c*x))
y <- yexact+0.2*runif(np) # compute a y
# y <- yexact+0.1*runif(np) # compute a y
df<-data.frame(x=x, y=y)
plot(x,y) # for information
st<-c(a=1, b=1,c=c) # set the "default" starting vector
n0<-try(nls(form, start=st, data=df)) # and watch the fun as this fails. ??DOES NOT FAIL!
summary(n0)
