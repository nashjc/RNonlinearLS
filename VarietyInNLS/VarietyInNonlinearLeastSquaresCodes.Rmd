...
that is, minimize an objective function with respect to a set of $n$ parameters, $x$, where the 
...
There is no exogenous data here.

There may be additional conditions that add to the complexity of
the problem such as 
...
where the exogenous data is a single vector of numbers $y$. In nonlinear regression, we are mostly
interested in "fitting a model to data", and the "minimizing a sum of squares" is the approach.
...
parameters. That is, problems can be considered "nonlinear" if we cannot compute the best parameter values
in a pre-defined fixed number of computations and must use an iterative approach.

In what follows, we will primarily take the numerical analyst's viewpoint. That is, we will be looking
mostly at the calculations for the minimization of the sum of squares. However, the purpose of these
calculations can dictate much of the structure of the software in which such computations appear.
...
above.  (See *https://en.wikipedia.org/wiki/Newton%27s_method*).
...
derivatives of the Hessian. However, there have been suggestioned
Newton-like approaches, possibly for special types of models where some short cut
to the Hessian is available. These may or may not make sense in the context of
...
least squares methods. The `maxLik` package (@maxLik2011) is a prime example.
...
- tools such as the `nls()` function in R has so many legacy uses that it is likely
  impossible to deprecate it. However, it has also (as we detail in ??RefactoringNLS??)
  non-maintainable, but it seems unlikely that any
