...
(@cnm79, Example 12.2) shows
a slight advantage for `nlsralt` in execution timing with an equivalent solution.
...
of the Gauss-Newton. Clearly it is equivalent to augment $J$ by **both** $\lambda D$
and $\lambda \phi 1_n$.
...
provide an explicit (and fixed) parameter scaling.

?? manual scaling -- is it part of nlsr??
...
That is, we assume a generalized inverse of $J$ can be applied to both sides of the Gauss-Newtx

To move to the Marquardt variant of the Gauss-Newton, we need to augment $J$. Let us call this augmented matrix
$K$. We will also need to augment $r$ by an appropriate number of zeros. 
For the traditional Marquardt method, we
